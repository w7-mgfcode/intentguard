...
- `app.py` — the entry point (`python -m intentguard.app`): resolves `INTENTGUARD_*` settings, configures logging, loads the predictor eagerly (so startup fails before the port binds), and hands off to Uvicorn. The only module that reads the process environment to decide how to run.
