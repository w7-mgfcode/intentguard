...
Test metrics are computed from the **reloaded** artifact, never from the in-memory model, and the test split is read exactly once per run. All hyperparameters are frozen in `configs/default.toml` and validated before any test read, so no metric here can have influenced model selection.
...
E03 writes JSON only. Markdown rendering, calibration, coverage, selective risk, and latency reporting belong to later umbrellas, and none of those have been measured.
