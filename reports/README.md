...
Empty bins are recorded for transparency but excluded from the average. Because each bin is weighted by its own share of the examples, an empty bin contributes exactly zero to ECE whether or not it is included — the dilution worth guarding against is the *unweighted* one: averaging the per-bin gaps over all 15 bins instead of over the occupied ones scales that mean by exactly `non_empty / bin_count`. It flatters whichever model leaves more bins empty, which here is the worse-calibrated transformer: 9 of 15 bins occupied means a 40.0% understatement, against 0.0% for the baseline, whose 15 bins are all occupied. Two ECEs are comparable only when computed under the same binning.
